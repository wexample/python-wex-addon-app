...
class Migration_6_0_15__1(AbstractMigration):
    ...
    def apply(self, context: MigrationContext) -> None:
        for compose_path in self._iter_compose_files(context):
            ...
            updated = content.replace("RUNTIME_DOMAINS_STRING", "APP_DOMAINS_STRING")
            updated = updated.replace("RUNTIME_EMAIL", "APP_EMAIL")
            if updated != content:
    ...
    def rollback(self, context: MigrationContext) -> None:
        for compose_path in self._iter_compose_files(context):
            content = compose_path.read_text()
            updated = content.replace("APP_DOMAINS_STRING", "RUNTIME_DOMAINS_STRING")
