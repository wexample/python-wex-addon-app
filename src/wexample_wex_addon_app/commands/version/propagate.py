...
def app__version__propagate(
    ...
    if isinstance(app_workdir, FrameworkPackageSuiteWorkdir):
        context.kernel.log(f"Propagating version of suite's packages {app_workdir.get_project_name()}")
        # Propagate versions
        app_workdir.propagate_packages_versions()
    else:
        context.kernel.log(f"Propagating version of single package {app_workdir.get_project_name()}")
        # Propagate versions
